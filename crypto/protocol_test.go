...
func TestProtocol_NotRegistered(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "no engine registered for") || !strings.Contains(err.Error(), "github.com/charlienet/go-crypto/crypto/asym") {
		t.Fatalf("NewAsymmetric(INVALID) 错误不符: %v", err)
	}

	_, err = crypto.NewKeyAgreement(crypto.AsymmetricAlgorithm("INVALID"))
	if err == nil || !strings.Contains(err.Error(), "no engine registered for") || !strings.Contains(err.Error(), "github.com/charlienet/go-crypto/crypto/agreement") {
		t.Fatalf("NewKeyAgreement(INVALID) 错误不符: %v", err)
	}

	_, err = crypto.GenerateKeyPair(crypto.AsymmetricAlgorithm("INVALID"))
	if err == nil || !strings.Contains(err.Error(), "no engine registered for") || !strings.Contains(err.Error(), "github.com/charlienet/go-crypto/crypto/keymgr") {
